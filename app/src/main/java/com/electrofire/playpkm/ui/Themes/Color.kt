package com.electrofire.playpkm.ui.Themes

import androidx.compose.ui.graphics.Color

val CelesteAzulado = Color(0xFF5FD8EC)
val Blanco = Color(0xFFFFFFFF)
val CelesteAzuladoDos = Color(0xFF2AA1E6)
val AzulOscuro = Color(0xFF30509C)
val AzulMarino = Color(0xFF002545)
val verdeAgua = Color(0xFF29f49a)
val rojo = Color(0xFFff332d)
val grisNuevo = Color(color = 0xFFBABECF)
val grisOscuro = Color(color = 0xFF606060)
val amarillo = Color(0xFFFFC107)
val amarilloAnaranjado = Color(0xFFFF9800)
